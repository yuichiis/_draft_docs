...
class Blas
{
        ...
        BufferInterface $Y, int $offsetY, int $incY ) : void
    {
        ...
        if($trans==BLAS::NoTrans || $trans==BLAS::ConjNoTrans ) {
            $rows = $m; $cols = $n;
        } elseif($trans==BLAS::Trans || $trans==BLAS::ConjTrans) {
            $rows = $n; $cols = $m;
        } else {
        ...
        BufferInterface $C, int $offsetC, int $ldC ) : void
    {
        ...
        // Check Buffer A
        if($transA==BLAS::NoTrans || $transA==BLAS::ConjNoTrans) {
            $rows = $m; $cols = $k;
        } else if($transA==BLAS::Trans || $transA==BLAS::ConjTrans) {
            $rows = $k; $cols = $m;
        } else {
            throw new InvalidArgumentException('unknown transpose mode for bufferA.');
        }
        ...
        // Check Buffer B
        if($transB==BLAS::NoTrans || $transB==BLAS::ConjNoTrans) {
            $rows = $k; $cols = $n;
        } elseif($transB==BLAS::Trans || $transB==BLAS::ConjTrans) {
            $rows = $n; $cols = $k;
        } else {
        ...
        BufferInterface $C, int $offsetC, int $ldC ) : void
    {
        ...
        if($trans==BLAS::NoTrans || $trans==BLAS::ConjNoTrans) {
            $rows = $n; $cols = $k;
        } else if($trans==BLAS::Trans || $trans==BLAS::ConjTrans) {
            $rows = $k; $cols = $n;
        } else {
            throw new InvalidArgumentException("unknown transpose mode for bufferA.");
        }
